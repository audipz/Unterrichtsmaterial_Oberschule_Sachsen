...
public class UserProvisioningService {
    ...
    public CreateUserResult create(CreateUserCommand command) {
        ...
        if (command.roles().contains(UserRole.SYSTEM_ADMIN)) {
            throw new SecurityException("SYSTEM_ADMIN cannot be assigned by a school admin");
        }
        ...
        userAccountPort.createUser(userId, command.schoolId(), username, displayName, normalizedDisplayName,
                passwordHash, Set.copyOf(command.roles()), true, command.createdBy());
    ...
    private String generateAvailableDisplayName(UUID userId, Set<UUID> classIds) {
        ...
        for (int attempt = 0; attempt < MAX_DISPLAY_NAME_ATTEMPTS; attempt++) {
            ...
            if (!displayNameConflictPort.conflictsInClasses(userId, normalized, visibleClasses)) return candidate;
        }
    ...
    private static void validate(CreateUserCommand command) {
        if (command == null) throw new IllegalArgumentException("Benutzeranlage darf nicht leer sein.");
        if (command.schoolId() == null) throw new IllegalArgumentException("Schule ist erforderlich.");
        if (command.username() == null || command.username().isBlank()) throw new IllegalArgumentException("Benutzername ist erforderlich.");
        if (command.initialPassword() == null || command.initialPassword().isBlank()) throw new IllegalArgumentException("Startpasswort ist erforderlich.");
        if (command.roles() == null || command.roles().isEmpty()) throw new IllegalArgumentException("Mindestens eine Rolle ist erforderlich.");
    }
}
