...
class UserProvisioningServiceTest {
    ...
    void createsUserWithHashedPasswordRolesAndGeneratedDisplayName() {
        ...
        var authorized = new AtomicReference<UUID>();

        UserAccountPort accountPort = new UserAccountPort() {
            @Override
            public boolean usernameExists(UUID school, String username) { return false; }

            @Override
            public void createUser(UUID userId, UUID school, String username, String displayName,
                                   String displayNameNormalized, String passwordHash, Set<UserRole> roles,
                                   boolean mustChangePassword, UUID actor) {
                captured.set(new CapturedUser(userId, school, username, displayName, displayNameNormalized,
        ...
        SchoolAuthorizationPort authorizationPort = new SchoolAuthorizationPort() {
            @Override
            public void requireSchoolAdmin(UUID actorId, UUID school) {
                assertThat(school).isEqualTo(schoolId);
                authorized.set(actorId);
            }
            @Override
            public void requireSystemAdmin(UUID actorId) { throw new UnsupportedOperationException(); }
        };

        var service = new UserProvisioningService(accountPort, passwordHashPort, conflictPort, authorizationPort);

        var result = service.create(new CreateUserCommand(
                schoolId, "s7a-0184", "Start!123", Set.of(UserRole.STUDENT), Set.of(classId), createdBy));

        CapturedUser user = captured.get();
        assertThat(authorized.get()).isEqualTo(createdBy);
    ...
    private record CapturedUser(UUID userId, UUID schoolId, String username, String displayName,
                                String displayNameNormalized, String passwordHash, Set<UserRole> roles,
                                boolean mustChangePassword, UUID createdBy) {}
}
