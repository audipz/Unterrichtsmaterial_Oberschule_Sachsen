...
public final class ClassAdministrationService {
    ...
    public UUID createClass(UUID schoolId, String name, int gradeLevel, String schoolYear, UUID actorId) {
        ...
        if (name == null || name.isBlank() || schoolYear == null || schoolYear.isBlank()) throw new IllegalArgumentException("name and schoolYear must not be blank");
    ...
    public void removeStudent(UUID schoolId, UUID classId, UUID studentId, LocalDate effectiveDate, UUID actorId) {
        ...
        requireSameSchool(schoolId, user.schoolId());
        requireRole(user.roles(), "STUDENT");
        port.removeStudent(schoolId, classId, studentId, effectiveDate == null ? LocalDate.now() : effectiveDate, actorId);
    }

    public void moveStudent(UUID schoolId, UUID studentId, UUID sourceClassId, UUID targetClassId, LocalDate effectiveDate, UUID actorId) {
        authorizationPort.requireSchoolAdmin(actorId, schoolId);
        if (sourceClassId.equals(targetClassId)) throw new IllegalArgumentException("sourceClassId and targetClassId must differ");
        var user = port.requireActiveUser(studentId);
        requireSameSchool(schoolId, user.schoolId());
        requireRole(user.roles(), "STUDENT");
        ensureDisplayNameAvailable(user, targetClassId, Set.of(sourceClassId));
        port.moveStudent(schoolId, studentId, sourceClassId, targetClassId, effectiveDate == null ? LocalDate.now() : effectiveDate, actorId);
    }
    ...
    private void ensureDisplayNameAvailable(ClassAdministrationPort.UserContext user, UUID targetClassId, Set<UUID> excludedClasses) {
    ...
    private static void requireSameSchool(UUID expected, UUID actual) {
        if (!expected.equals(actual)) throw new IllegalArgumentException("user belongs to another school");
    }

    private static void requireRole(Set<String> roles, String requiredRole) {
        if (!roles.contains(requiredRole)) throw new IllegalArgumentException("user does not have role " + requiredRole);
    }
}
